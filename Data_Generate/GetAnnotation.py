import os
import sys
sys.path.insert(0,os.getcwd())  # os.getcwd()获取当前程序绝对路径，并加入搜索路径，优先级最高（为0）
from utils.train_utils import get_info

'''annotations.txt中信息大致如下
类别   标签
------------
daisy 0
dandelion 1
...
def main():
    classes_path    = 'datas/annotations.txt'   # 标签信息
    datasets_path   = 'datasets'        # 数据集位置
    datasets        = ["train", "test"]
